...
export type 对话消息 = {
  ...
  是否我方: boolean;
  是否系统消息?: boolean; // 谈判状态变更等系统提示
  内容: string;
  创建时间: string;
};

// 谈判状态白名单（前后端硬编码一致）
export type 谈判状态 = "议价中" | "已达成" | "已搁置" | "已拒绝";
...
export type 对话 = {
  ...
  // 谈判语义（null 表示普通聊天）
  谈判状态?: 谈判状态 | null;
  成交价_cents?: number | null;
  成交价_currency?: string | null;
  // 列表预览字段
  最后消息预览?: string | null;
  最后消息时间?: string | null;
  未读数?: number;
...
// 谈判进度变更（PATCH /api/conversations/{id}/谈判进度）
export async function 更新谈判进度(
  对话id: string,
  参数: { 状态: 谈判状态; 成交价_cents?: number; 成交价_currency?: string },
): Promise<对话> {
  // 路径含中文 — encodeURI 转义
  const 路径 = encodeURI(`/api/conversations/${对话id}/谈判进度`);
  return 请求<对话>(
    路径,
